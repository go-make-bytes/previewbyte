...
// readyz is readiness: the render engine pool can hand out a live instance. It
// returns a plain {status} body — never an error envelope — so an orchestrator
// probe gets a uniform readiness signal, and skips the access line/span.
func (r *router) readyz(ctx *azugo.Context) {
	ctx.SkipRequestLog()
	if err := r.Renderer().Ready(ctx); err != nil {
		notReady(ctx, "render engine")
