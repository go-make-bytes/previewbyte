...
`go-platform-kit` v1.11.3, `go-authbyte` v0.23.1 and `go-sec-events` v1.2.1, plus the image
library `golang.org/x/image` to v0.46.0. No endpoint, field, error or setting changes with them,
nothing in your configuration needs touching, and rendering behaviour is unchanged. Two crossed
